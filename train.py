...
#    x_train, t_train, x_test, t_test = train_test_split_tdnn(x_seq, t_seq, TEST_SIZE)
    ...
    x = tf.compat.v1.placeholder(dtype=tf.float32, shape=(None, DELAY_SIZE, input_nm))
    t = tf.compat.v1.placeholder(dtype=tf.float32)
...
#        self.sett(flag, model, DELAY_SIZE)
    ...
    return flag, model, DELAY_SIZE, anticipation_size, midVel, avgVel